...
protein_list <- c('P04217')

protein_list <- read.csv('uniprot_ids.csv')
```
...
edges <- merged |>
  ...
  dplyr::select(uniprot_id, protein_name, target = targets) |>
  tidyr::drop_na() |>
...
```
```{r}
locations <- dplyr::select(locations, -c('protein_name_alt', 'gene_name_alt'))
